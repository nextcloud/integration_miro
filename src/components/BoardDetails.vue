				...
				<div class="buttons">
					...
					<a :href="boardLink" target="_blank">
						<NcButton>
							...
							{{ t('integration_miro', 'Open in a new tab') }}
						</NcButton>
					</a>
					<div v-if="talkEnabled"
						...
						<NcButton @click="showTalkModal = true">
							...
							{{ t('integration_miro', 'Share link to a Talk conversation') }}
						</NcButton>
				...
				<div class="link">
					...
					<div class="rightPart linkInputWrapper">
						...
						<a :href="boardLink" @click.prevent.stop="copyLink(false)">
							<NcButton v-tooltip.bottom="{ content: t('integration_miro', 'Copy to clipboard') }">
								<template #icon>
				...
				<div v-for="(field, fieldId) in fieldsToDisplay"
					...
					<div class="rightPart">
						<label v-if="['ncCheckbox'].includes(field.type)"
							...
							{{ board[fieldId] ? t('integration_miro', 'Enabled') : t('integration_miro', 'Disabled') }}
						</label>
	...
	methods: {
		async copyLink() {
			...
			try {
				...
				console.error(error)
				showError(t('integration_miro', 'Link could not be copied to clipboard'))
			}
		},
